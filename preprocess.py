...
def predata_x(path, num_rows, start_num):
	...
	x = np.zeros((num_rows, 240, 320, 3))
	for i in range(num_rows):
		if i % 200 ==0:
			print ("preprocessed: " + str(i))
		path = name[i]
		img = load_img(path, target_size=(240, 320))
...
if __name__=="__main__":
	...
	# x_train1 = predata_x('./train/', 2000, 1)
	# np.save('./data2/x_train', x_train1)
	...
	# np.save('./data2/x_val', x_val)
	# print ('x val saved')
	# print (x_val)

	# x_test = predata_x('./test/', 1531, 1)
	# np.save('./data2/x_test', x_test)
	# print ('x test saved')
	# print (x_test.shape)

	y_train = predata_y('./train_labels.csv')
	np.save('./data2/y_train', y_train)
	print ('y train saved')
	print (y_train.shape)
	# y_val = predata_y('./validation.csv')
	# np.save('./data2/y_val', y_val)
	# print ('y val saved')
	# print (y_val.shape)